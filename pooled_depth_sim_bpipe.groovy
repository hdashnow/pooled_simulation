...
// Takes mapped exome bams. Simulates pooled exomes by downsampling reads but maintaining the same ploidy
...
downsampling = [1, 2, 3, 4, 5, 6] // i.e. 1 is no downsampling, 2 is 1/2, 3 is 1/3 etc.
set_pool = {
//    println(inputs)
    branch.downsampling = branch.name.toInteger()
    branch.num_samples = 6
...
//@filter('downsampled')
downsample_region = {
    produce(output.prefix+'.downsampled'+branch.downsampling+'.bam') {
        output.dir="align"
        DOWNSAMPLE=7 + 1.0/branch.downsampling
        exec """
...
merge_bams = {
    produce('merge.'+branch.downsampling+'.bam') {
        output.dir="align"
        exec """
